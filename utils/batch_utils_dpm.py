...
class patch_generator:
    ...
    def __iter__(self):
        while True:
            ...
            for path_id in self.path_id_generator:
                ...
                input_img, label_img = self.get_img_at(path_id,randnum)
                if len(input_img) > 0:
    ...
    def get_img_at(self, index, randnum):
        ...
        # print(randnum)
        if randnum == 1:
            target_img_full = self.load_img(path).astype("float32")
        else:
            target_img_full = self.load_img(path.replace("target-Pol", "target-BF")).astype("float32")
        # target_img_full = self.load_img(path).astype("float32")
        
        input_img_full = normalize_input(input_img_full)

        year_num = randnum
        year_matrix = np.ones_like(input_img_full)*year_num
        input_img_full = np.concatenate((input_img_full, np.expand_dims(year_matrix[0,:,:],axis=0)), axis=0)
...
class Dataset_test(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #     "float32"
        # )
        input_img = np.load(os.path.join(self.image_dir, tile_id + ".npy")).astype(
            "float32"
        )
        ...
        year_num = -1
        year_matrix = np.ones_like(input_img)*year_num
        input_img = np.concatenate((input_img, np.expand_dims(year_matrix[0,:,:],axis=0)), axis=0)
