...
from utils.reggan.layers import DownBlock, Conv, ResnetTransformer
...
# If len(ndf[cfg]) > len(nuf[cfg]) - then the deformation field is up-sampled
...
# Indicate if the a final refinement layer is applied on the before deriving
...
class Reg(nn.Module):
    ...
    def forward(self, img_a, img_b):
        return self.offset_map(img_a, img_b)
