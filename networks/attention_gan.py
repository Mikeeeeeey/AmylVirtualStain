...
def weights_init(m):
    if isinstance(m, nn.Conv2d):
        ...
        # nn.init.kaiming_uniform_(m.weight.data, a=0.1, mode='fan_in',
...
# UnetUpBlock with group norm instead of batch norm
class UNetDownBlock(nn.Module):
    def __init__(self, in_size, out_size, padding=1, batch_norm=True):
        super(UNetDownBlock, self).__init__()
...
class SimpleConv(nn.Module):
    def __init__(self, in_size, padding=1, batch_norm=True):
        ...
        block.append(
            nn.Conv2d(
                in_size, in_size * 2, kernel_size=4, stride=2, padding=int(padding)
            )
        )
        if batch_norm:
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        # encoding path, assuming input x in (in_ch,256,256),
        # first channel num = 64;
        ...
        # attention gate, x is from skip connection in down path,
        # g is output of previou up block.
...
class Discriminator(nn.Module):
    def __init__(self, config, in_channels=1, padding=1, batch_norm=True):
        ...
        for i in range(config.n_blocks):
            ...
            in_size = in_size * 2
        # conv_path += [
        #     nn.Conv2d(in_size, in_size, kernel_size=3, stride=2, padding=1),
        #     nn.Conv2d(in_size, 1, kernel_size=4, stride=2,
