...
from utils.batch_utils import Dataset_train, Dataset_test
...
class GAN(pl.LightningModule):
    def __init__(self, config):
        ...
        self.criterionGAN = GANLoss(use_lsgan=False)  # Define GAN loss function
        self.criterionL1 = nn.SmoothL1Loss(reduction="mean")  # Define L1 loss function
        self.criterionSSIM = SSIM_loss(size_average=True)  # Define SSIM loss function
        self.criterionTV = TVLoss()  # Define TV loss function

        # Cache for generated images
        self.last_inps = None
        self.last_imgs1 = None
        self.last_imgs2 = None
        self.last_lbls1 = None
        self.last_lbls2 = None
    ...
    def validation_step(self, batch, batch_nb):
        ...
        if batch_nb == 0:
            ...
            self.last_imgs1 = generated[:img_save_num, 0:3, :, :]
            self.last_imgs2 = generated[:img_save_num, 3:7, :, :]
            self.last_lbls1 = real_image[:img_save_num, 0:3, :, :]
            self.last_lbls2 = real_image[:img_save_num, 3:7, :, :]
        self.log_dict(
    ...
    def validation_epoch_end(self, outputs):
        ...
        output_grid_1 = torchvision.utils.make_grid(
            self.last_imgs1, nrow=1, normalize=True, value_range=(-1, 1)
        )
        output_grid_2 = torchvision.utils.make_grid(
            self.last_imgs2, nrow=1, normalize=True, value_range=(-1, 1)
        )
        label_grid_1 = torchvision.utils.make_grid(
            self.last_lbls1, nrow=1, normalize=True, value_range=(-1, 1)
        )
        label_grid_2 = torchvision.utils.make_grid(
            self.last_lbls2, nrow=1, normalize=True, value_range=(-1, 1)
        )
        tb_logger.experiment.add_image(
            "valid",
            torch.cat([input_grid, output_grid_1, output_grid_2, label_grid_1, label_grid_2], dim=2),
            self.current_epoch,
        )
    ...
    def test_step(self, batch, batch_nb):
        ...
        for i, fname in enumerate(image_names):
            img = self.norm_output(bf[i])
            if not os.path.exists(self.config.output_images_dir + "-BF"):
                os.mkdir(self.config.output_images_dir + "-BF")
            img.save(self.config.output_images_dir + "-BF" + "\\" + fname + ".png")
            img = self.norm_output(pol[i])
            if not os.path.exists(self.config.output_images_dir + "-POL"):
                os.mkdir(self.config.output_images_dir + "-POL")
            img.save(self.config.output_images_dir + "-POL" + "\\" + fname + ".png")
